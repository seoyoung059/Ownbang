...
const RealEstateDetail = ({ item, onOpenReservationCard }) => {
    ...
    const images = item && item.roomImageResponses ? item.roomImageResponses.map((image) => image.roomImageUrl) : [];
    ...
    return (
        <Box sx={styles.container}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                <Button variant="contained" sx={styles.button} onClick={onOpenReservationCard}>
                    예약하기
                </Button>
            </Box>
        </Box>
